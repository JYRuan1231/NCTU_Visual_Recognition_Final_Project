...
#                     loss = criterion(outputs, targets)
                    ...
                    if (iter+1) % 500 ==0:
                            print('{} iter:{} cls_loss: {:.4f} rg_loss {:.4f} ord_loss {:.4f} cls_Acc: {:.4f} rg_Acc: {:.4f} Score {:.4f}'.format(phase, iter, 
                                                                                                                 cls_loss,
                                                                                                                 rg_loss,
                                                                                                                 ord_loss,                                 
                                                                                              train_corrects.double() / ((iter+1)*batch_size),
                ...
                print('{} Loss: {:.4f}  Acc: {:.4f} rg_acc: {:.4f} Score: {:.4f}'.format(
...
if __name__ == "__main__":
    ...
    print('Number of training data:',n_train,
          '\nNumber of validation data:',n_valid)

    train_dataset = PseudoDataset(csv_file=cfg.train_csv, file_id=train_id, transform=data_transforms['train'],
                              mode='train', id_conversion=id_conversion)
    valid_dataset = PseudoDataset(csv_file=cfg.train_csv, file_id=valid_id, transform=data_transforms['test'],
                              mode='valid', id_conversion=id_conversion)
    ...
    ex_train_dataset = PseudoDataset(csv_file=cfg.pseudo_csv, file_id=ex_train_id,
                                 transform=data_transforms['train'], mode='pseudo', id_conversion=id_conversion)
    ex_test_dataset = PseudoDataset(csv_file=cfg.pseudo_csv, file_id=ex_valid_id,
                                transform=data_transforms['test'], mode='pseudo', id_conversion=id_conversion)
    ...
    print('Number of pseudo training data:', train_size*cfg.batch_size,
      '\nNumber of pseudo validation data:', valid_size)
    ...
    with torch.cuda.device(0):
        ...
        optimizer_ft = optim.SGD(model_ft.parameters(), lr=cfg.learning_rate, momentum=cfg.momentum, weight_decay=cfg.weight_decay)

        # # Decay LR by a factor of 0.1 every 30 epochs

        # exp_lr_scheduler = lr_scheduler.StepLR(optimizer_ft, step_size=10, gamma=0.1)
        exp_lr_scheduler = lr_scheduler.CosineAnnealingWarmRestarts(optimizer_ft, T_0=cfg.epochs, T_mult=cfg.T_mult, eta_min=cfg.eta_min)

        model_ft = train_model(model_ft, criterion, optimizer_ft, exp_lr_scheduler, num_epochs=cfg.epochs)

        ...
        for phase in ["train", "val"]:
            ...
            if phase == "train":
                for iter, (inputs, targets) in tqdm(
                    ...
                    if (iter + 1) % 500 == 0:
                        print(
                            "{} iter:{} cls_loss: {:.4f} rg_loss {:.4f} ord_loss {:.4f}"
                            "cls_Acc: {:.4f} rg_Acc: {:.4f} Score {:.4f}".format(
                                phase,
                                iter,
                                cls_loss,
                                rg_loss,
                                ord_loss,
                                train_cls_acc.double()
                                / ((iter + 1) * batch_size),
                                train_rg_acc.float()
                                / ((iter + 1) * batch_size),
                ...
                print(
                    "{} Loss: {:.4f}  cls_acc: {:.4f} rg_acc: {:.4f} Score: {:.4f}".format(
                ...
                if (epoch + 1) % 5 == 0 and (epoch + 1) >= 20:
                    ...
                    torch.save(
                        epoch_model,
                        "./models/" + model_name + "_" + str(epoch + 1),
                    )
...
if __name__ == "__main__":
    with torch.cuda.device(1):
        ...
        exp_lr_scheduler = lr_scheduler.CosineAnnealingWarmRestarts(
            optimizer_ft,
            T_0=scheduler_exp_t0,
            T_mult=scheduler_exp_t_mult,
            eta_min=scheduler_exp_eta_min,
        )
